<?php
$this->Html->addCrumb('My Hotel Reviews', 'javascript:void(0);', array('class' => 'breadcrumblast'));
...
<div class="row"> 
    ...
    <div class="col-sm-12">
                ...
                <?php
                echo $this->Form->create('TravelHotelLookup', array('paramType' => 'querystring' , 'url' => '/travel_hotel_reviews/index', 'class' => 'quick_search', 'id' => 'SearchForm', 'type' => 'post', 'novalidate' => true, 'inputDefaults' => array(
                        'label' => false,
                        'div' => false,
                        'class' => 'form-control',
                ),array('controller' => 'travel_hotel_reviews'),
                    ));
                ...
                ?> 
                    ...
                    <div class="col-sm-3 col-xs-6">
					 ...
					 <div class="col-sm-3 col-xs-6">
                        <label for="un_member">Review Status:</label>
                        <?php echo $this->Form->input('is_page', array('options' => $PageStatus, 'empty' => '--Select--', 'value' => $is_page, 'data-required' => 'true')); ?>
                    </div>
					
					<div class="col-sm-3 col-xs-6">
                        <label for="un_member">Mapping Status:</label>
                        <?php echo $this->Form->input('mapping_status', array('options' => $MappingStatus, 'empty' => '--Select--', 'value' => $mapping_status, 'data-required' => 'true')); ?>
                    </div>
</br>  
            ...
            <table width="800" style="margin-left:auto;margin-right:auto cellpadding="0" cellspacing="0" id="resp_table" class="table toggle-square myclitb" data-filter="#table_search" data-page-size="1000">
                <thead>
                    <tr>
                        ...
                        <th width="29%" data-group="group1" data-sort-ignore="true">Address</th>
                        ...
                        <th width="4%" data-group="group1" data-sort-ignore="true">Review?</th>
...
//echo count($TravelHotelLookups);
                    if (isset($TravelHotelLookups) && count($TravelHotelLookups) > 0):
                        foreach ($TravelHotelLookups as $TravelHotelLookup):
                            ...
                            if($TravelHotelLookup['TravelHotelLookup']['is_page'] == 'Y')
                                $page_flag = 'Yes';
                            else 
                                $page_flag = 'No';
                            ?>
					...
					if($TravelHotelLookup['TravelHotelLookup']['is_image'] == 'Y'){
						 echo $this->Html->link('<span class="icon-eye-open"></span>', 'view/' . $id , array('class' => 'act-ico','target' => '_blank', 'escape' => false));
					}else{
						 echo $image_flag;	
					}
                                ...
                                <td class="tablebody"><?php echo $page_flag; ?></td>               
                                <td valign="middle" align="center">

                                    <?php
									$countError = $this->Custom->getWTBErrorHotalReview($id);
										if($countError > 0){
									...
									if($TravelHotelLookup['TravelHotelLookup']['is_page'] == 'Y'){
										 echo $this->Html->link('<span class="icon-eye-open"></span>', 'view/' . $id , array('class' => 'act-ico','target' => '_blank', 'escape' => false));
											...
											echo $this->Html->link('<span class="icon-pencil"></span>', 'edit/' . $id."/continent_id:$continent_id/country_id:$country_id/province_id:$province_id/city_id:$city_id/is_page:$is_page/mapping_status:$mapping_status" , array('class' => 'act-ico','target' => '_blank', 'escape' => false));	
										}
									}
...
$this->Js->get('#TravelHotelLookupCountryId')->event('change', $this->Js->request(array(
            'controller' => 'all_functions',
            'action' => 'get_review_province_by_continent_country/TravelHotelLookup/continent_id/country_id'
                ), array(
